...
import {
  loadWelcomePage,
  submitWelcomePage,
  getWelcomePageToggle,
  setWelcomePageToggle,
...
const WelcomePageEditor = () => {
  ...
  useEffect(() => {
    if (session?.user?.organizationId) {
      loadWelcomePage(session.user.organizationId.toString())
        .then((response) => {
          if (response.data.welcomePage) {
            ...
            console.error("Error loading welcome page:", response.data.error);
          }
        })
        .catch((error) => console.error("API request error:", String(error)));
      getWelcomePageToggle(session.user.organizationId.toString())
        .then((response) => {
          if (response.data.welcomePageToggle !== undefined) {
            setEdit(response.data.welcomePageToggle);
          } else {
            console.error(
              "Error loading welcomePageToggle:",
              response.data.error
            );
          }
        })
  ...
  const handleSubmitPage = async () => {
    ...
    try {
      const response = await submitWelcomePage(
        session.user.organizationId.toString(),
        pageContent
      );

      if (response.data.message) {
        console.log("Successfully updating welcome page");
        setSaved(true);
      } else {
        console.error("Error updating welcome page:", String(response.data.error));
      }
    } catch (error) {
  ...
  const handleToggleChange = async () => {
    if (session?.user?.organizationId) {
      try {
        const response = await setWelcomePageToggle(
          session.user.organizationId.toString(),
          !edit
        );
        if (response.data.message) {
          console.log("Successfully updated welcomePageToggle");
        } else {
          console.error("Error updating welcomePageToggle:", response.data.error);
        }
      } catch (error) {
        console.error("API request error:", String(error));
      }
    ...
    <div className="flex-column mx-1 my-2 flex gap-8 rounded-sm p-4">
      <div>
        <div className="flex items-start gap-4">
          <h2 className="text-lg font-bold">Welcome Page</h2>
          <ToggleSwitch
        ...
        <div className="flex items-start justify-between">
          <p className="m-0 mb-1 h-6 text-sm font-medium font-medium text-gray-900 text-slate-600 dark:text-gray-300">
            Design a Welcome page for approved new members
          </p>
          {edit && (
            <div
        ...
        <div className="pb-3">
          <Toast>
            ...
            <div className="pl-2 text-sm font-normal">
              Welcome Page Saved Successfully!
            </div>
            <Toast.Toggle />
          </Toast>
        </div>
...
export default AdminAuthWrapper(WelcomePageEditor);
