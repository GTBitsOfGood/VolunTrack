...
export const createOrganizationValidator = object().shape({
  ...
  notification_email: string()
    ...
    default_zip: string().trim().matches(/^[0-9]+$/, "Must be a number"),
    default_contact_name: string().trim(),
    default_contact_email: string().trim().email("Invalid email"),
    default_contact_phone: string().trim(),
    
    event_silver: string().trim().matches(/^[0-9]+$/, "Must be a number"),
    event_gold: string().trim().matches(/^[0-9]+$/, "Must be a number"),
    hours_silver: string().trim().matches(/^[0-9]+$/, "Must be a number"),
    hours_gold: string().trim().matches(/^[0-9]+$/, "Must be a number"),    
