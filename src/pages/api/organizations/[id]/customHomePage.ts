...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (!id ) {
        return res.status(400).json({ message: "Missing or invalid organization ID." });
    }
        ...
        switch (req.method) {
            ...
            case "GET": {
              ...
              if (!organization.homePage) {
                  return res.status(404).json({message: "Organization home page not found."})
              }
              
              return res
                .status(200)
                .json({ homePage:organization.homePage });
            }
            ...
            case "POST": {
                ...
                if (!homePage) {
                    return res.status(400).json({ error: "Missing required field: homePage" });
                }
